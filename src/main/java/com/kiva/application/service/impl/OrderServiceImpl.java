...
import com.kiva.application.entity.*;
...
import com.kiva.application.model.dto.OrderDetailDTO;
import com.kiva.application.model.dto.OrderInfoDTO;
import com.kiva.application.model.request.CreateOrderRequest;
import com.kiva.application.model.request.UpdateDetailOrder;
import com.kiva.application.model.request.UpdateStatusOrderRequest;
...
import com.kiva.application.repository.StatisticRepository;
...
public class OrderServiceImpl implements OrderService {
	...
	public Order createOrder(CreateOrderRequest createOrderRequest, long userId) {
		...
		order.setPayment(createOrderRequest.getPayment());
		order.setShipping(createOrderRequest.getShipping());
	...
	public OrderDetailDTO userGetDetailById(long id, long userId) {
		...
		if (order.getPayment() == 2) {
			order.setPaymentText("COD");
		} else if (order.getPayment() == 1) {
			order.setPaymentText("VNPay");
		}
		Long productPrice = order.getProductPrice();
		String shippingPrice = order.getShipping();
		float floatValue = Float.parseFloat(shippingPrice);
		int intValue = (int) (floatValue * 1000);
		Long totalPrice = order.getTotalPrice();
		Long discount = totalPrice - productPrice - intValue;
