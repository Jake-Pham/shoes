...
import com.kiva.application.model.dto.ChartDTO;
...
//    @Query(value = "SELECT NEW com.kiva.application.model.dto.ProductInfoDTO(p.id, p.name, p.slug, p.price ,p.images ->> '$[0]', p.total_sold) " +
//            "FROM product p " +
//            "WHERE p.status = 1 " +
